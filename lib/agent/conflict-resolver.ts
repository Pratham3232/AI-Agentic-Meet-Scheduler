...
export async function resolveConflict(
  ...
  // Run all fallback strategies in parallel to minimise latency
  const [expandResult, adjacentResult, weekdayResult] = await Promise.all([
    expandTimeWindow(params, debug, timezone),
    tryAdjacentDays(params, debug, timezone),
    tryNextWeekdays(params, debug, timezone),
  ]);
...
async function tryNextWeekdays(
  ...
  // Query all candidates in parallel, then return the earliest with slots
